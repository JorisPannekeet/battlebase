...
export function getTargets(state, targetQuery) {
  ...
  throw new Error(
    `Could not find target "${targetQuery}" on ${state.dungeon.y}/${state.dungeon.x}`
  );
}
...
export function isRoomCompleted(room) {
  ...
  } else if (room.type === "start" || room.type === "quest") {
    return true;
  }
...
// Checks if the whole stage (all rooms) has been cleared.
// As long as there is one cleared node per floor.
export function isStageCompleted(state) {
